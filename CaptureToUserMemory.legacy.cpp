...
int ANGIO_GAIN = 1;
int ANGIO_CONTRAST = 1;
int ANGIO_RED_RATIO_PRE = 1;
int ANGIO_BLUE_RATIO_PRE = 1;

int ANGIO_RED_GAIN_POST = 1;
int ANGIO_GREEN_GAIN_POST = 1;
int ANGIO_BLUE_GAIN_POST = 1;
int ANGIO_TOTAL_GAIN_POST = 1;
...
//-----------------------------------------------------------------------------
void displayImage( CaptureParameter* pCaptureParameter, Request* pRequest, bool freeze=false )
//-----------------------------------------------------------------------------
{
...
#endif // #ifdef USE_DISPLAY
#ifdef BUILD_WITH_OPENCV_SUPPORT
    int openCVDataType = CV_8UC1; // after all the switched it will be 0
    switch( pRequest->imagePixelFormat.read() ) // gives 1 on our device
    {
    case ibpfMono8:
        openCVDataType = CV_8UC1;
        break;
...
//    cout << "openCVDataType " << openCVDataType << endl;
    cv::Mat openCVImage( cv::Size( pRequest->imageWidth.read(),
...
////    cout << "pCaptureParameter->openCVResultDisplayTitle " << pCaptureParameter->openCVResultDisplayTitle << endl;
...
//
////    cv::Mat local_im = *image;
//    cv::Mat edgesMat;
////    cv::Canny( *image, edgesMat, 35.0, 55.0 );
//    cv::imwrite("my_image.png", image);
////    cv::imshow("canny edges", edgesMat);
//    cv::waitKey(0);
//}


void take_picture(CaptureParameter* pParameter, ObjectsHolder* o, Request* ir_pRequest){
    cout << "inside take_picture" << endl;
    adjust_motor();
    free_locked_requests(o, ir_pRequest);
    stop_acquisition(o, pParameter);
    angio_settings(o);
    cout << "finished angio_settings" << endl;
    ...
    if (pRequest != nullptr){
        if (pRequest->isOK()){
            ...
            cv::Mat openCVImage = cv::Mat(
                    ...
                    pRequest->imageData.read(),
                    pRequest->imageLinePitch.read());
            cout << "picture is ready!" << endl;
//            vector<int> compression_params;
//            compression_params.push_back(cv::CV_IMWRITE_PNG_COMPRESSION);
//            compression_params.push_back(9);
            string im_filename = "/home/spring/im.jpg";
            cout << "saving as " << im_filename << endl;
            cv::imshow("what was taken here?", openCVImage);
            cv::waitKey( 0 );
            bool res = cv::imwrite(im_filename, openCVImage);
//            bool res = cv::imwrite(im_filename, openCVImage, compression_params);
            cout << "imwrite result is: "<< res << endl;
            cout << "image is saved as "<< im_filename << endl;
//            return openCVImage;
//            algorithm(openCVImage);
            // avihu algo
            // avihu algo
            cout << "preparing for chauduri algorithm" << endl;
            EReport rv(ER_SUCCESS);
            cv::Mat X, Y, im, dst, dst1;
            std::vector<cv::Mat> kernels;
            std::vector<cv::Mat> kers;
            std::string filename = "/opt/mvIMPACT_Acquire/apps_test/CaptureToUserMemory/arm64/newINI.ini";
...
//            bgr[1].copyTo(im);
            cv::Mat bgr[4];
            cout << "openCVImage.channels() = " << openCVImage.channels() << endl;
            cout << "openCVImage.size().width, openCVImage.size().height" << openCVImage.size().width << openCVImage.size().height << endl;
            cv::split(openCVImage, bgr);
            bgr[1].copyTo(openCVImage);

            std::cout << "im.type(): " << im.type() << std::endl;
            cout << "initializing chauduri algorithm" << endl;
...
//            double e1, e2;
            ...
            //{
//            e1 = cv::getTickCount();
            cout << "starting chauduri algorithm..." << endl;
            rv = CImProcAlgo::getReference().springImProcAlgo(im, dst);
            // avihu algo
            // avihu algo
//            cv::Mat edgesMat;
//            cv::Canny(openCVImage, edgesMat, 35.0, 55.0 );
            cout << "finished chauduri algorithm" << endl;
//            cv::imshow("canny edges", edgesMat);
            if (ER_SUCCESS == rv)
            {
                cv::imshow("chauduri", dst);
            } else {
                cout << "\ngot error from algorithm   :(   err_num: " << rv << "\n" << endl;
            }
//            cv::imwrite("canny_edges.png", edgesMat);

        }else{
            cout << "somthing went wrong again" << endl;
//            openCVImage = nullptr;
        }
        pRequest->unlock();
//        return openCVImage;
...
//-----------------------------------------------------------------------------
{
    ...
    ObjectsHolder* o = pThreadParameter->o;
    ...
    while( !s_boTerminated )
    {
        ...
        if( pRequest )
        {
            ...
            if(line_is_active(4, o)){
                cout << "line 4  active!!" << endl;
                break;
//                take_picture(pThreadParameter, o, pRequest);
//                cv::Mat* cv_pic = take_picture(pThreadParameter, o, pRequest);
...
//        }
    ...
    cout << "finished acquisition loop" << endl;
